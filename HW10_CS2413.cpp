...
void Pre_Traverse(Node* head) // Pre-Order Traverse
{
	cout << head->Get_key(); // Output the current node

	if(head->Get_left() != nullptr) // Check if Left subtree exists and if so traverse it
	{
		Pre_Traverse(head->Get_left());
	}
	if(head->Get_right() != nullptr) // Check if Right subtree exists and if so traverse it
	{
		Pre_Traverse(head->Get_right());
	}
}

void Post_Traverse(Node* head) // Post-Order Traverse
{
	if(head->Get_left() != nullptr) // Check if Left subtree exists and if so traverse it
	{
		Post_Traverse(head->Get_left());
	}
	
	if(head->Get_right() != nullptr) // Check if Right subtree exists and if so traverse it
	{
		Post_Traverse(head->Get_right());
	}

	cout << head->Get_key(); // Output the current node
}

void In_Traverse(Node* head) // In-Order Traverse
{
	if(head->Get_left() != nullptr) // Check if Left subtree exists and if so traverse it
	{
		In_Traverse(head->Get_left());
	}
	...
	if(head->Get_right() != nullptr) // Check if Right subtree exists and if so traverse it
	{
		In_Traverse(head->Get_right());
	}
}

void BF_Traverse(Node* head) // Breadth-First Traverse
{
	queue<Node*> Q;
...
{
	if(s == "pre") // Pre-Order Traverse
		Pre_Traverse(p);
	else if(s == "post") // Post-Order Traverse
		Post_Traverse(p);
	else if(s == "in") // In-Order Traverse
		In_Traverse(p);
	else if(s == "bf") // Breadth-First Traverse
		BF_Traverse(p);
	else
		cout << -1;	// Incorrect input
}
...
Node* Convert2Array(Node* root, int height) 
{
	...
	while(!Q.empty()) // Continues while there are still Nodes to output
	{
		Node current = Q.front();
		Q.pop();
		arr[i] = current;
		i++;
		if(current.Get_left() != nullptr) // Adds left subtree if it exists
			Q.push(*current.Get_left());
		if(current.Get_right() != nullptr) // Adds right subtree if it exists
			Q.push(*current.Get_right());
	}
