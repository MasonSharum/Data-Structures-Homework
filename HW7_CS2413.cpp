...
// The hash function that determines where the key will be placed
// in the table.
...
// Creates a table that will store all of the students
// and initializes every cell to NULL
...
// Searches the table for key. Returns NULL if it is not found
// otherwise it returns the address of key.
...
// Removes the student with value key from the table if it is found
void HashTable::Remove(int key)
{
	...
	else
	{
		...
		{
			Node* temp = table[index]->Get_p();
			delete table[index];
			table[index] = temp;
			return;
		}
		else if(table[index]->Get_p() != NULL)
		{
			Node* temp = table[index]->Get_p();
