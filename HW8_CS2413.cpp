...
int HashTable::hash(int key) //Hashes the key
{
	return (key % div);
}

void HashTable::CreateTable(int divisor) //Creates the table
{
...
int HashTable::Search(int key) /*Searches the table for the key, returning index if found 
									and -1 otherwise */
{
...
void HashTable::Add(Node temp) //Adds a student to the table resizing if necessary
{
	int tempKey = temp.Get_key();
	int hashed = hash(tempKey);
	bool First = true;
	int prev = 0;

	if(table[hashed].Get_key() == -1) //Hashed cell is empty, so we just add the key
	{
		table[hashed].Set_key(tempKey);
	}
	else
	{
		int end = 0;
		...
		for(int i = 0; i < size; i++) //checks through every cell in the table until an empty cell is found
		{
			if(table[i].Get_key() == -1)
			{
				table[i].Set_key(tempKey);
				table[prev].Set_index(i);
				prev = i;
				return;
			}
			end++;
		...
		if(end == size) /* If no cell is found then it resizes the table and re-adds 
							all of the previous values */
		{
			int newSize = size * 2;
			...
			for(int i = 0; i < size; i++) //It again searches for an empty cell
			{
				if(table[i].Get_key() == -1)
				{
					table[i].Set_key(tempKey);
...
{
	for(int i = 0; i < size; i++) //Searches the table
	{
		if(table[i].Get_key() == key) //If key is found it removes it
		{
			table[i].Set_key(-1);
		}
	}
...
int HashTable::Get_Size() //Returns the current size of the table
{
	return size;
}
